    ...
    float* d_mean, float* d_std) 
{
    ...
    {}
        ...
        //遍历tar
        int tar_idx = y*tarW+x;
        int tar_area = tarH*tarW;


        //遍历src
        int src_idx =src_y*srcW+src_x;
    ...
    float* d_mean, float* d_std) 
{
    ...
    if (src_y1 < 0 || src_x1 < 0 || src_y2 > srcH || src_x2 > srcW) {
        ...
        // bilinear interpolation -- 计算每个点的权重
    ...
    float* d_mean, float* d_std) 
{
    ...
    //找到临近四个点
    int src_x1 =floor((float)(x+0.5)*scaled_w-0.5);
    int src_y1 =floor((float)(y+0.5)*scaled_h-0.5);
    int src_x2 = src_x1+1;
    int src_y2 =src_y1+1;

    if(src_x1<0||src_y1<0||src_x2>srcW||src_y2>srcH)
    {}
    else
    {
    ...
    //算权重
    float srcx1y1=(1-th)*(1-tw);//左上
    float srcx2y1 =(1-th)*tw;
    float srcx1y2=th*(1-tw);
    float srcx2y2 = th*tw;

    //算偏移

    x =x +int(tarW/2)- int(srcW/scaled_w*2);
    y =y +int(tarH/2)- int(srcH/scaled_h*2);
    //如果是按w的缩放比例的化，x还是以前的x，y进行偏移，同理如果时y的缩放比例，那x偏移y不动
    //算tar索引
    ...
    //算src每个点的索引
    int srcx1y1_idx = (src_y1*srcW+src_x1)*3;
    int srcx1y2_idx=(src_y2*srcW+src_x1)*3;
    int srcx2y1_idx = (src_y1*srcW+src_x2)*3;
    int srcx2y2_idx =(src_y2*srcW+src_x2)*3;


    //分配，变成[RRRRRRRRRRRRRRRRRRRRRR][GGGGGGGGGGGGGGGGG][BBBBBBBBBBBBBBBBBBBBBBBB]
    tar[tar_idx+tar_area*0]=(round((src[srcx1y1_idx+2]*srcx1y1+
                                    src[srcx1y2_idx+2]*srcx1y2+
                                    src[srcx2y1_idx+2]*srcx2y1+
                                    src[srcx2y2_idx+2]*srcx2y2))/255.0f-d_mean[2])/d_std[2];

   tar[tar_idx+tar_area*1]=(round((src[srcx1y1_idx+1]*srcx1y1+
                                    src[srcx1y2_idx+1]*srcx1y2+
                                    src[srcx2y1_idx+1]*srcx2y1+
                                    src[srcx2y2_idx+1]*srcx2y2))/255.0f-d_mean[1])/d_std[1];

   tar[tar_idx+tar_area*2]=(round((src[srcx1y1_idx+0]*srcx1y1+
                                    src[srcx1y2_idx+0]*srcx1y2+
                                    src[srcx2y1_idx+0]*srcx2y1+
                                    src[srcx2y2_idx+0]*srcx2y2))/255.0f-d_mean[0])/d_std[0];
    }
