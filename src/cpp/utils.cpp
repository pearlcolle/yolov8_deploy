...
vector<unsigned char> loadFile(const string &file){
    ...
    in.seekg(0, ios::end);//到达文件末尾
    size_t length = in.tellg();//测量长度

    vector<uint8_t> data;//创建一个容器
    if (length > 0){
        in.seekg(0, ios::beg);//跳到开头
        data.resize(length);//容器扩展那么大
        in.read((char*)&data[0], length);//把数据读进容器中
    }
    in.close();//关闭
    return data;//返回容器
}
